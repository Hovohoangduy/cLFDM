...
class LoRALinear(nn.Module):
    ...
    def forward(self, x):
        ...
        lora_update = (x @ self.lora_A.T) @ self.lora_B.T * self.scale
        return base * lora_update
    
class FlowDiffusionLoRA(FlowDiffusion):
    def __init__(self, *args, lora_r=4, lora_alpha=15, **kwargs):
        super.__init__(*args, **kwargs)
        for name, module in self.unet.named_modules():
            if isinstance(module, nn.Linear) and module.out_features == module.in_features:
                lora = LoRALinear(module.in_features, module.out_features, r=lora_r, alpha=lora_alpha, bias=(module.bias is not None))
                lora.weight.data.copy_(module.weight.data)
                if module.bias is not None:
                    lora.bias.data.copy_(module.bias.data)
                parent = module._get_name().split('.')
                setattr(self.unet, name, lora)
        
        for param in self.unet.parameters():
            param.requires_grad = False
        for module in self.unet.modules():
            if isinstance(module, LoRALinear):
                module.lora_A_requires_grad = True
                module.lora_B.requires_grad = False
        self.optimizer = torch.optim.Adam(
            [p for n, p in self.unet.named_parameters() if p.requires_grad],
            lr=1e-4
        )
    
    def train_step(self, *args, **kwargs):
        self.optimizer.zero_grad()
        loss_dict = self.forward(*args, **kwargs)
        loss = loss_dict['loss']
        loss.backward()
        self.optimizer.step()
        return loss
